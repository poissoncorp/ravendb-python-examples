...
class Employees_ByWorkPeriod(AbstractIndexCreationTask):
    ...
    def __init__(self):
        super().__init__()
        self.map = "docs.Employees.Select(employee => new {" "    working_in_company_since = employee.HiredAt.Year" "})"
...
class ProjectIndexResults(Example):
    def run(self, run_params: RunParams):
        start_year = run_params.start_year or 1993
        ...
        with self.document_store_holder.store().open_session() as session:
            ...
            ).where_greater_than("working_in_company_since", start_year)
            # endregion
            # region Step_6
