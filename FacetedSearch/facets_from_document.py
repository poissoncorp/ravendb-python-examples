...
class FacetsFromDocument(Example):
    def run(self, run_params: RunParams):
        facets_results = []
        range_1 = run_params.range_1 or 25
        range_2 = run_params.range_2 or 50
        range_3 = run_params.range_3 or 100

        # region Demo
        with self.document_store_holder.store().open_session() as session:
            # region Step_2
            facet_setup = FacetSetup()
            facet_setup.id_ = "myFacetSetupDocumentID"
            ...
            range_facet.ranges = [
                f"PricePerUnit < {range_1}",
                f"PricePerUnit >= {range_1} and PricePerUnit < {range_2}",
                f"PricePerUnit >= {range_2} and PricePerUnit < {range_3}",
                f"PricePerUnit > {range_3}",
            ]
            facet_setup.range_facets = [range_facet]
            # endregion

            # region Step_3
            session.store(facet_setup, facet_setup.id_)
