...
class CreateCompareExchange(Example):
    def run(self, run_params: RunParams) -> str:
        cmp_xchg_key = run_params.cmp_xchg_key or "abc@gmail.com"
        cmp_xchg_value = run_params.cmp_xchg_value or "employee/1-A"
        result = None

        # region Demo
        # region Step_1
        put_cmp_xchg_operation = PutCompareExchangeValueOperation(cmp_xchg_key, cmp_xchg_value, 0)
        ...
        # region Step_3
        get_cmp_xchg_operation = GetCompareExchangeValueOperation(cmp_xchg_key, str)
