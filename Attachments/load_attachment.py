...
class LoadAttachment(Example):
    ...
    def run(self, run_params: RunParams) -> None:
        ...
        with self.document_store_holder.store().open_session() as session:
            # region Step_1
            attachment_exists = session.advanced.attachments.exists(document_id, attachment_name)
            # endregion

            if attachment_exists:
                # region Step_2
                with session.advanced.attachments.get(document_id, attachment_name) as attachment_result:
                    # endregion
                    # region Step_3
